...
```
공격자는 당시 등록되어 있지 않은 의미 없는 긴 문자열의 도메인을 코드에 하드코딩 해 두었습니다.
보안 분석 환경(Sandbox)은 가짜 인터넷 연결을 제공하여 모든 도메인 접속을 성공으로 처리하는 경우가 많습니다. 이를 이용해 분석 환경임을 감지하면 실행되지 않도록 설계한 것으로 추측됩니다.
### 2.2. 네트워크 연결 시도
```c
v4 = InternetOpenA(0, 1u, 0, 0, 0); 
InternetOpenUrlA(v4, szUrl, 0, 0, 0x84000000, 0);
```
+ `InternetOpenA` : 인터넷 연결을 초기화합니다.
+ `InternetOpenUrlA` : 1.1에서 설정한 szURL에 대한 연결을 설정하고 요청을 보냅니다.
    + 이 함수는 도메인이 응답하면 핸들을 반환하고, 실패하면 `NULL`을 반환합니다.

### 2.3. 실행 분기점
의사코드에는 생략되어 있으나 어셈블리 코드를 확인하면 아래와 같습니다.
```assembly
...
킬 스위치를 통과하면 시스템에 자신을 고착화하고 실제 랜섬웨어 본체를 꺼냅니다.

### 3.1. 서비스 제어 및 분기 [(sub_408090)](./Pseudocode/sub_408090)
이 함수는 프로그램의 실행 모드를 결정하는 메인 컨트롤러 역할을 합니다.
+ 인자값 체크 : `_p___argc() < 2 `를 통해 실행 시 인자값이 있는지 확인합니다. 인자가 없다면 처음 실행된 것으로 간주하고 설치 루틴(`sub_407F20`)을 호출합니다.
+ 서비스 시작 : 인자가 있다면 `StartServiceCtrlDispatcherA`를 호출하여 자신을 Windows 서비스로 구동시킵니다. 이때 실제 서비스 로직인 [`sub_408000`](./Pseudocode/sub_408000)이 실행됩니다.

### 3.2 서비스 등록 [(sub_407C40)](./Pseudocode/sub_407C40)
시스템이 재부팅되어도 악성코드가 계속 실행되도록 자신을 서비스로 등록합니다.

+ 자신을 실행할 때 `-m security`라는 인자를 붙이도록 설정합니다.
```c
  sprintf(Buffer, "%s -m security", FileName);
```
+ `CreateServiceA`를 통해 자신을 **"Microsoft Security Center (2.0) Service"**라는 이름의 서비스로 등록합니다.
+ 등록하자마자 `StartServiceA`를 호출하여 서비스를 즉시 시작합니다.
...
+ 파일 드롭 : `C:\WINDOWS\tasksche.exe` 경로에 파일로 생성(Drop)합니다.
    + 만약 기존 파일이 있다면 `MoveFileExA`를 이용해 이름을 `qeriuwjhrf`로 바꾸어 백업하거나 덮어씁니다.
+ 최종 실행 : `CreateProcessA`를 호출하여 추출한 `tasksche.exe`를 실행합니다.

## 4. 네트워크 전파 메커니즘 [(sub_408000)](./Pseudocode/sub_408000)
이 함수는 표준적인 Windows 서비스 메인 함수의 형태를 띠고 있습니다.
```mermaid
graph TD
...
1. 서비스 상태 설정 : `ServiceStatus.dwCurrentState = 2;` (SERVICE_START_PENDING)로 시작하여 핸들러로 등록한 뒤 `4`(SERVICE_RUNNING)로 변경합니다.
2. 핵심 실행 루틴 호출 : 서비스가 실행중 상태(`4`)가 되자마자 [`sub_407BD0()`](#41-멀티스레드-전파-엔진-sub_407bd0)를 호출합니다.
3. 지연 및 종료 : `Sleep(0x5265C00u);` 약 24시간 동안 대기한 후 프로세스를 종료합니다.

### 4.1. 멀티스레드 전파 엔진 [(sub_407BD0)](./Pseudocode/sub_407BD0)

#### 4.1.1 로컬 네트워크 스캐닝 [(sub_407720)](./Pseudocode/sub_407720)
이 함수는 현재 감염된 PC와 같은 네트워크에 있는 다른 PC들을 찾아냅니다.

+ 정보 수집 [(sub_409160)](./Pseudocode/sub_409160) : 감염됨 PC의 IP 주소와 서브넷 마스크 정보를 가져옵니다.
...
```
+ 공격 스레드 생성([`sub_4076B0`](./Pseudocode/sub_4076B0)) : 로컬 IP 리스트를 순회하며 `sub_4076B0` 함수를 실행합니다. 이 함수가 445번 포트를 통해 SMB 취약점을 이용합니다.

#### 4.1.2 공인 IP 무작위 스캐닝 [(sub_407840)](./Pseudocode/sub_407840)
이 함수는 무작위 IP 주소를 생성하여 공격 대상을 찾습니다.
+ 랜덤 IP todtjd (`srand`,`sub_407660`): `GetTickCount`와 `Time` 등을 조합해 난수 시드를 만들고 무작위 IP 주소를 생성합니다.
+ 필터링 : 루프 내에서 특정 대역은 피합니다.
    + `v6 == 127` : 루프백 주소 제외
    + `v6 >= 224` : 멀티캐스트 및 예약된 IP 대역 제외.
+ 서브넷 응답 스레드 생성 : `sub_407480` 함수로 무작위로 하나의 IP를 찍어보고 응답이 있다면 해당 대역의 마지막 자리(1~254)를 전부 훑으며 공격 스레드를 생성합니다.
```c
sprintf(Buffer, "%d.%d.%d.%d", v6, v16, v7, v10); // 마지막 옥텟(v10)을 1부터 254까지 증가
```
### 4.2. 445번 포트 노킹 [(`sub_407480`)](./Pseudocode/sub_407480)
이 함수는 특정 IP의 TCP 445번 포트(SMB)가 열려 있는지 확인하는 스캐너입니다.

+ 타겟 포트 설정
```c
 *(_WORD *)name.sa_data = htons(0x1BDu); //0x1BDu는 10진수로 445
```
+ `sub_407840`이 `1`(성공)을 반환하면 `StartAddress` 함수를 스레드로 실행합니다. 이 함수가 **EternalBlue(MS17-010)** 익스플로잇을 동작합니다.

## 5. SMB 취약점 공격 지휘 [(StartAddress)](./Pseudocode/StartAddress)
이 함수는 [`백도어 확인` -> `취약점 공격` -> `페이로드 전송`] 과정을 반복합니다
```c
...
```
+ `0x1BDu`(445번 포트)를 대상으로 공격을 수행하는 루틴입니다.
+ 최대 5회(`i < 5`)재시도하는 로직을 통해 공격의 성공률을 높이려 합니다.
...
```
+ 응답 패킷(`buf`)의 34번째 오프셋(MID Field)이 `81`(0x51)인지 체크합니다.
+ 이는 알려진 DoublePulsar 백도어의 Ping 응답 규약과 일치합니다.
...
if ( send(v3, byte_42E4F4, 78, 0) != -1
          ...
          && buf[12] == -64 )   // 0xC0
```
+ 또한 `sub_401980`에서는 `buf[9]` 부터 `buf[12]`까지 4바이트(Status Field)를 체크하고 있습니다. 이를 리틀 엔디언 방식으로 합치면 `0xC0000205` 값이 나옵니다.
+ `0xC0000205`는 Windows의 NT_STATUS 코드 중 `STATUS_INSUFF_SERVER_RESOURCES`를 의미합니다.
+ 이 값을 반환하면 대상 PC는 SMB 취약점에 대한 업데이트가 되지 않은 경우(취약한 경우)라고 판단합니다.


+ 따라서 이 함수는 타겟에 이미 백도어가 심어져 있는지 판별하는 역할을 수행한다고 결론지을 수 있습니다.
...
```
+ `v2` 구조체에 담긴 SMB 템플릿과 `double`형을 이용한 정밀한 타이밍 제어가 특징입니다.
+ 이는 SMB 프로토콜의 취약점을 이용해 커널 메모리 내에서 Race Condition을 유발하기 위한 전형적인 설계로 보입니다.
...
```
+ 백도어 응답 패킷의 특정 오프셋(18, 22)에서 데이터를 읽어 침투한 시스템의 운영체제 정보를 파악합니다.
+ 파악된 정보를 바탕으로 `sub_406EB0`에서 적절한 쉘코드를 생성합니다.
+ 최종적으로 백도어를 통해 워너크라이 복제본(`mssecsvc.exe`)을 타겟의 메모리에 injection하는 최종 단계입니다.

#### 4.5.1 [(sub_406F50)](./Pseudocode/sub_406F50)
실제 injection point 입니다.
+ 쉘 코드 및 데이터 준비
```c
if ( a2 ) { // x86 경로
    v3 = *(const void **)&FileName[260]; // 악성 파일 데이터
    v4 = 4869; // 쉘 코드 크기
    v5 = &unk_506000; // 쉘 코드 템플릿
} else { // x64 경로
    v4 = 6144;
    v5 = &unk_50D800;
}
```
> 아키텍처(`a2`)에 따라 메모리에 미리 로드된 쉘 코드 템플릿(`v5`)과 공격 본체(`v3`)를 가져옵니다.
> 이후 `GlobalAlloc`으로 메모리를 할당한 뒤, [쉘 코드 + 악성 파일 데이터]를 병합합니다.
