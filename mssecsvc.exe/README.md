...
```
+ 응답 패킷(`buf`)의 34번째 바이트가 `81`(0x51)인지 체크한다.
+ 이는 DoublePulsar 백도어의 Ping 응답 규약과 일치한다.
...
if ( send(v3, byte_42E4F4, 78, 0) != -1
          ...
          && buf[12] == -64 )   // 0xC0
```
+ 또한 `sub_401980`에서는 `buf[9]` 부터 `buf[12]`까지 4바이트를 체크하고 있다. 이를 리틀 엔디언 방식으로 합치면 `0xC0000205` 값이 나온다.
